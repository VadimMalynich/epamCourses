...
public class SimpleSelectionSortTest {
    ...
    @Test(description = "Positive scenario of simple selection sort from min to max value", dataProvider = "testSortToHighProvider")
    public void testSortToHigh(int[] arr, int[] res) throws ServiceException {
        Array actual = new Array(arr);
        Array result = new Array(res);
        selectionSort.sort(actual, true);
        assertEquals(actual, result);
    }

    @Test(description = "Positive scenario of simple selection sort from max to min value", dataProvider = "testSortToLowProvider")
    public void testSortToLow(int[] arr, int[] res) throws ServiceException {
        Array actual = new Array(arr);
        Array result = new Array(res);
        selectionSort.sort(actual, false);
        assertEquals(actual, result);
    }

    @Test(description = "Negative scenario of simple selection sort when array has length < 2",
            expectedExceptions = ServiceException.class,
            expectedExceptionsMessageRegExp = "Array size is less or equal 1. Insertion sorting impossible to apply",
