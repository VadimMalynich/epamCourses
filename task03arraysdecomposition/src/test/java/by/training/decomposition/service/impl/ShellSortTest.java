...
public class ShellSortTest {
    ...
    @Test(description = "Positive scenario of shell sorting from min to max value", dataProvider = "testSortToHighProvider")
    public void testSortToHigh(double[] arr, double[] res) throws ServiceException {
        ...
        shellSort.sort(actual, true);
        assertEquals(actual, result);
    }

    @Test(description = "Positive scenario of shell sorting from max to min value", dataProvider = "testSortToLowProvider")
    public void testSortToLow(double[] arr, double[] res) throws ServiceException {
        Array actual = new Array(arr);
        Array result = new Array(res);
        shellSort.sort(actual, false);
        assertEquals(actual, result);
    }

    @Test(description = "Negative scenario of shell sorting when array has length < 2",
            expectedExceptions = ServiceException.class,
            expectedExceptionsMessageRegExp = "Array size is less or equal 1. Shell sort impossible to apply",
